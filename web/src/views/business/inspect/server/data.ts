...
export const excludeServerTags = [
  '离线',
  '裁撤',
  '关停',
  '下架',
...
export const searchFormSchema = (onIdcTypeChange, onCategoryIdsChange): FormSchema[] => [
  {
    field: 'categoryIds',
    label: '业务大类',
    component: 'ApiSelect',
  ...
  {
    field: 'excludeServerTags',
    label: '排除标签',
    component: 'Select',
    componentProps: {
      options: excludeServerTags.map((item) => ({
        label: item,
        value: item,
      })),
      maxTagCount: 4,
    ...
    defaultValue: excludeServerTags,
    helpMessage:
      '当设备的标签包含任意所选条目时，该设备将不会显示在查询结果中。当下拉选项中不包含期望的标签时，可手动输入标签并按回车确认。',
    colProps: { span: 6 },
  },
];
...
export const inspectRecordSearchFormSchema = (
  // onIdcTypeChange,
  // onCategoryIdChange,
  onTimePikerOpen,
  checkFieldsChange,
...
export const inspectRecordColumns: BasicColumn[] = [
  {
    title: '任务ID',
    dataIndex: 'taskId',
    width: 200,
