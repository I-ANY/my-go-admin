...
<script lang="ts" setup>
  ...
  const [registerTable, { setSelectedRowKeys, getSelectRowKeys, setProps }] = useTable({
    title: '需求单列表',
    api: getMatchingDemandList,
    columns: normalDemandColumns(),
    beforeFetch: (params) => {
    ...
    rowSelection: {
      ...
      getCheckboxProps: (record) => {
        ...
        const isFutureMonth = record.startTime
          ? dayjs(record.startTime).isAfter(dayjs().endOf('month'))
          : false;

        return {
          disabled: isBwInsufficient || isFutureMonth,
        };
      },
    },
