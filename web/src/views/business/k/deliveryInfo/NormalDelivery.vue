<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" v-auth="'business:k:batchDelivery2'" @click="handleBatchDelivery"
          >批量交付</a-button
        >
        <a-button
          type="primary"
          v-auth="'business:k:normal:batchBindDemand'"
          ...
          <template #overlay>
            <Menu>
              <MenuItem
                ...
                v-if="hasPermission('business:k:normal:delivery:batchClose')"
              >
                批量关闭交付
              </MenuItem>
              <MenuItem
                ...
                v-if="hasPermission('business:k:normal:delivery:batchEdit')"
              >
                批量编辑
              </MenuItem>
            </Menu>
          ...
          <TableAction
            ...
            :dropDownActions="[
              ...
              },
              {
                label: '关闭交付',
                disabled: !canEdit(record),
                auth: 'business:k:normal:delivery:close',
                popConfirm: {
                  title: '是否确认关闭交付',
                  placement: 'left',
    ...
    <NormalDemandSelectModal
      @register="registerModal"
      @success="handleSuccess"
      :biz-type="bizType"
    />
    <NormalBatchDemandModal
      @register="registerNormalBatchDemandModal"
      @success="handleSuccess"
      :biz-type="bizType"
    />
    <NormalDeliveryInfoModal
...
<script lang="ts" setup>
  ...
  import NormalDemandSelectModal from './NormalDemandSelectModal.vue';
  import NormalBatchDemandModal from './NormalBatchDemandModal.vue';
  import NormalDeliveryInfoModal from './NormalDeliveryInfoModal.vue';
  import DeliveryInfoMacModal from './DeliveryInfoMacModal.vue';
  import DifIspModal from './DifIspModal.vue';
  import NormalBatchEditModal from './NormalBatchEditModal.vue';
  ...
  const [registerNormalBatchDemandModal, { openModal: openNormalBatchDemandModal }] = useModal();
  ...
  defineOptions({ name: 'KNormalDelivery' });
  const { hasPermission } = usePermission();
  const { notification } = useMessage();
  const bizType = BizType.normal;
  const [registerTable, { getSelectRowKeys, getSelectRows, reload, setSelectedRowKeys }] = useTable(
    {
      title: '设备交付',
      ...
      actionColumn: {
        ...
        ifShow: function (): boolean {
          return (
            hasPermission('business:k:delivery2') ||
            hasPermission('business:k:demand:edit2') ||
            hasPermission('business:k:info:edit2') ||
            hasPermission('business:k:normal:difIsp') ||
            hasPermission('business:k:normal:delivery:close')
          );
        },
      },
    },
  ...
  function showDropdown() {
    return hasPermission('business:k:normal:delivery:batchClose');
  }
  async function handlebatchBindDemand() {
    ...
    openNormalBatchDemandModal(true, data);
  }
