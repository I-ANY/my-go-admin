    ...
    <Tabs v-model:activeKey="activeKey" @change="handleTabChange" destroyInactiveTabPane>
      <template v-for="tab in tabs" :key="tab.key">
        <Tabs.TabPane :tab="tab.label" :key="tab.key" v-if="hasPermission(tab.permission)">
          <template v-if="tab.key === 'biz-category'">
            <BizCategory />
          </template>
          <template v-if="tab.key === 'biz-group'">
            <BusinessGroup />
          </template>
        </Tabs.TabPane>
      </template>
...
<script lang="ts" setup>
  import BizCategory from './biz.vue';
  import BusinessGroup from './group.vue';
  ...
  const tabs: any[] = [
    {
      label: '业务名管理',
      key: 'biz-category',
      permission: 'business:B:biz:category:view',
    },
    {
      label: '业务组管理',
      key: 'biz-group',
      permission: 'business:B:biz:group:view',
    },
  ];
