  ...
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button
          type="primary"
          v-auth="'business:lb:GetBillSummaryList:export'"
...
<script lang="ts" setup>
  ...
  import { columns, searchFormSchema, ispMap, billTypeMap, statusMap } from './data95';
  import { Api, GetBillSummaryList } from '@/api/business/lb';
  ...
  defineOptions({ name: 'LB95' });
  const [registerTable, { getForm }] = useTable({
    title: 'LB95 带宽值',
    api: GetBillSummaryList,
    columns,
    formConfig: {
      labelWidth: 120,
  ...
  function handleExportData() {
    Modal.confirm({
      ...
      async onOk() {
        ...
        await exportLbBillSummary(value);
      },
    });
  }
  onMounted(async () => {
  ...
  async function exportLbBillSummary(value: Recordable) {
    console.log('value', value);

    try {
      let filename = await downloadFileByUrl(Api.ExportBillSummary, 'POST', 5 * 60, value, null);
